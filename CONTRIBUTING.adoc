...
[NOTE]
====
There is currently one non-open source dependency `com.force.api:swag` due to which you may need this step. This will soon be removed as this project turns fully open source.

* You need to add your nexus credentials in `~/.gradle/gradle.properties` file (create the file if not present), like below:
...
* These are used by your local gradle to connect to Salesforce nexus for downloading artifacts.
