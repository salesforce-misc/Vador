...
abstract class BaseValidationConfig<ValidatableT, FailureT> {
    ...
    @Nullable
    protected Function1<SpecFactory<ValidatableT, FailureT>, @NonNull Collection<@NonNull ? extends BaseSpecBuilder<ValidatableT, FailureT, ?, ?>>> specify;
    @Singular("withSpec")
    ...
    List<BaseSpec<ValidatableT, FailureT>> getSpecs() {
        ...
        return Stream.concat(Stream.ofNullable(specify).flatMap(specs -> specs.apply(specFactory).stream().map(BaseSpecBuilder::done)),
                Stream.ofNullable(withSpecs).flatMap(specs -> specs.stream().map(spec -> spec.apply(specFactory).done()))).collect(Collectors.toList());
    }
