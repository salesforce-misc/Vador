...
class BatchRunnerDslTest {
    ...
    void failFastPartialFailures() {
        ...
        List<Validator<BaseParent, ValidationFailure>> validators = List.of(
                ...
                parent -> parent.map(BaseParent::getId).filterOrElse(predicateForValidId1, ignore -> VALIDATION_FAILURE_1),
                parent -> parent.map(BaseParent::getId).filterOrElse(predicateForValidId2, ignore -> VALIDATION_FAILURE_2)
        );
