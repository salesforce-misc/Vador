...
class Spec1Test {
    ...
    void spec1ShouldMatch() {
        final var validationConfig = ValidationConfig.<Bean1, ValidationFailure>toValidate().withSpec(spec ->
        ...
        final var noneResult = validateAndFailFast(validBean, ValidationFailure::getValidationFailureForException, validationConfig);
    ...
    void spec1ShouldMatchField() {
        final var validationConfig = ValidationConfig.<Bean2, ValidationFailure>toValidate().withSpec(spec ->
                spec._1().orFailWith(INVALID_COMBO_1)
    ...
    void spec1ShouldMatchAnyOfFields() {
        final var validationConfig = ValidationConfig.<Bean3, ValidationFailure>toValidate().withSpec(spec ->
                spec._1().orFailWith(INVALID_COMBO_1)
