...
class BatchRunnerDslTest {
    ...
    void ErrorAccumulateForSimpleValidators() {
        ...
        Assertions.assertTrue(result.get(2).forAll(resultPerValidation -> Either.right(new Parent(2, null)).equals(resultPerValidation)));
        ...
        result.take(2).forEachWithIndex((vf, index) -> 
                Assertions.assertTrue(vf.get(2).isRight() && Either.right(new Parent(index, null)).equals(vf.get(2))));
        ...
        result.takeRight(2).forEachWithIndex((vf, index) ->
                Assertions.assertTrue(vf.get(1).isRight() && Either.right(new Parent(validatables.size() - 2 + index, null)).equals(vf.get(1))));
