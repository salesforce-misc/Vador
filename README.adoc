...
and render a code-base which is challenging to test,
extend and maintain.
...
Use of a same Config pattern throughout, with self-explaining DSL methods to drive your development.
...
* Saves a lot of dev hours while writing and _10X_ more while reading.
...
* It's simple to switch orchestration strategy,
For example, if the current strategy is *Fail-Fast for Any*.
But if you want to migrate to Fail-Fast for each item (to handle partial failures) or if you have another route where you need to accumulate all errors, that's as simple as calling a different API method without changing anything else.
