...
This problem is a 3-dimensional design problem stretching among - Sub-requests, Service routes ( sharing common fields &nodes), and Validation count.
In the above imperative approach, we entangled all 3, which lead to chaos.
We need a design, which treats all of these separately, let them be extended independently, and abstracts out validation sequencing and orchestration.
...
Blog post: https://overfullstack.ga/posts/fight-complexity-with-fp/
...
=== 🍭 link:docs/config-dsls/configDSLs.adoc[Config DSLs]
...
*This is a powerful technique, which lets you validate any Bean with any level or nesting.
It's easy to fit this model in our heads, as validator configuration aligns with Bean hierarchical-structure*

image:hierarchical-validation.png[inline]
...
TIP: If you are using link:docs/config-dsls/configDSLs.adoc[`*Config` DSL] from 2.0, you don't need to worry about this, unless you are particular about the order of validators.
...
image:vader.gif[inline]

'''

== link:CONTRIBUTING.adoc[Wanna Contribute?]
