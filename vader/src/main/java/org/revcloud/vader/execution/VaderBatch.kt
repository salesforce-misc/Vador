...
package org.revcloud.vader.execution

import io.vavr.Function1
import io.vavr.Tuple
import io.vavr.Tuple2
import io.vavr.control.Either
import org.revcloud.vader.config.BatchOfBatch1ValidationConfig
import org.revcloud.vader.config.BatchValidationConfig
import org.revcloud.vader.config.base.BatchExecutionStrategy.FAIL_FAST_FOR_ANY
import org.revcloud.vader.config.base.BatchExecutionStrategy.FAIL_FAST_FOR_EACH
...
import org.revcloud.vader.execution.strategies.accumulationStrategy
import org.revcloud.vader.execution.strategies.failFastForAny
import org.revcloud.vader.execution.strategies.failFastForAnyBatchOfBatch1
import org.revcloud.vader.execution.strategies.failFastForEach
import org.revcloud.vader.execution.strategies.failFastForEachBatchOfBatch1
  ...
  companion object {
    ...
    fun <FailureT : Any, ContainerValidatableT> validateAndFailFastForContainer(
      ...
      .map { container: ContainerValidatableT ->
        Vader.validateAndFailFastForContainer(container, containerValidationConfig, throwableMapper)
    ...
    fun <FailureT : Any, ContainerValidatableT, PairT> validateAndFailFastForContainer(
      ...
      .map { container: ContainerValidatableT ->
        Vader.validateAndFailFastForContainer(container, containerValidationConfig, throwableMapper).map { Tuple.of(pairForInvalidMapper(container), it) }
      }.firstOrNull { it.isPresent } ?: Optional.empty()
    ...
    fun <FailureT : Any, ContainerValidatableT, NestedContainerValidatableT> validateAndFailFastForContainer(
      ...
      .map { container: ContainerValidatableT ->
        Vader.validateAndFailFastForContainer(container, containerValidationConfigWith2Levels, throwableMapper)
      }.firstOrNull { it.isPresent } ?: Optional.empty()
