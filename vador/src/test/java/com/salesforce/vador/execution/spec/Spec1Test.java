/*******************************************************************************
 * Copyright (c) 2022, salesforce.com, inc.
 * All rights reserved.
 * SPDX-License-Identifier: BSD-3-Clause
 * For full license text, see the LICENSE file in the repo root or https://opensource.org/licenses/BSD-3-Clause
 ******************************************************************************/

package com.salesforce.vador.execution.spec;

import static com.salesforce.vador.execution.Vador.validateAndFailFast;
import static com.salesforce.vador.matchers.AnyMatchers.anyOf;
import static com.salesforce.vador.matchers.IntMatchers.inRangeInclusive;
import static org.assertj.core.api.Assertions.assertThat;
import static sample.consumer.failure.ValidationFailure.INVALID_COMBO_1;
import static sample.consumer.failure.ValidationFailure.INVALID_VALUE;

import com.salesforce.vador.config.ValidationConfig;
import java.util.List;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import sample.consumer.failure.ValidationFailure;

class Spec1Test {

	@DisplayName("ShouldMatch")
	@Test
	void spec1ShouldMatch() {
		final var validationConfig =
				ValidationConfig.<Spec1Bean1, ValidationFailure>toValidate()
						.withSpec(
								spec ->
										spec._1()
												.orFailWith(INVALID_VALUE)
												.given(Spec1Bean1::getValue)
												.shouldMatch(anyOf(1, 2)))
						.prepare();
		final var invalidBean = new Spec1Bean1(3);
		final var failureResult = validateAndFailFast(invalidBean, validationConfig);
		assertThat(failureResult).contains(INVALID_VALUE);

		final var validBean = new Spec1Bean1(1);
		final var noneResult = validateAndFailFast(validBean, validationConfig);
		assertThat(noneResult).isEmpty();
	}

	@Test
	@DisplayName("Validator Types from Specs")
	void validatorTypesFromSpecs() {
		final var validationConfig =
				ValidationConfig.<Spec1Bean1, ValidationFailure>toValidate()
						.withSpec(
								spec ->
										spec._1()
												.orFailWith(INVALID_VALUE)
												.given(Spec1Bean1::getValue)
												.shouldMatch(anyOf(1, 2)))
						.prepare();
		assertThat(validationConfig.getValidatableType()).isEqualTo(Spec1Bean1.class);
	}

	@DisplayName("ShouldMatchAnyOf")
	@Test
	void spec1ShouldMatchAnyOf() {
		final var validationConfig =
				ValidationConfig.<Spec1Bean1, ValidationFailure>toValidate()
						.withSpec(
								spec ->
										spec._1()
												.orFailWith(INVALID_VALUE)
												.given(Spec1Bean1::getValue)
												.shouldMatchAnyOf(List.of(anyOf(1, 2), inRangeInclusive(3, 5))))
						.prepare();
		final var invalidBean = new Spec1Bean1(6);
		final var failureResult = validateAndFailFast(invalidBean, validationConfig);
		assertThat(failureResult).contains(INVALID_VALUE);

		final var validBean = new Spec1Bean1(3);
		final var noneResult = validateAndFailFast(validBean, validationConfig);
		assertThat(noneResult).isEmpty();
	}

	@DisplayName("ShouldMatchField")
	@Test
	void spec1ShouldMatchField() {
		final var validationConfig =
				ValidationConfig.<Spec1Bean2, ValidationFailure>toValidate()
						.withSpec(
								spec ->
										spec._1()
												.orFailWith(INVALID_COMBO_1)
												.given(Spec1Bean2::getField1)
												.shouldMatchField(Spec1Bean2::getField2))
						.prepare();
		final var invalidBean = new Spec1Bean2("", null);
		final var failureResult = validateAndFailFast(invalidBean, validationConfig);
		assertThat(failureResult).contains(INVALID_COMBO_1);

		final var validBean = new Spec1Bean2("", "");
		final var noneResult = validateAndFailFast(validBean, validationConfig);
		assertThat(noneResult).isEmpty();
	}

	@DisplayName("ShouldMatchAnyOfFields")
	@Test
	void spec1ShouldMatchAnyOfFields() {
		final var validationConfig =
				ValidationConfig.<Spec1Bean3, ValidationFailure>toValidate()
						.withSpec(
								spec ->
										spec._1()
												.orFailWith(INVALID_COMBO_1)
												.given(Spec1Bean3::getField1)
												.shouldMatchAnyOfFields(
														List.of(Spec1Bean3::getField2, Spec1Bean3::getField3)))
						.prepare();
		final var invalidBean = new Spec1Bean3("", null, "3");
		final var failureResult = validateAndFailFast(invalidBean, validationConfig);
		assertThat(failureResult).contains(INVALID_COMBO_1);

		final var validBean = new Spec1Bean3("", null, "");
		final var noneResult = validateAndFailFast(validBean, validationConfig);
		assertThat(noneResult).isEmpty();
	}
}
