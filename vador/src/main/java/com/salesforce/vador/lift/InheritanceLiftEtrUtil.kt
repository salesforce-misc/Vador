...
/**
 * Assume `ParentT` is parent by inheritance to `ValidatableT`. To validate `ValidatableT`, all it's
 * validations + all its Parent validations has to pass. These utils help to lift it's parent's
 * validations in the context of ValidatableT, so that they can be chained together.
 */
fun <ParentT, ValidatableT : ParentT, FailureT> liftToChildValidatorType(
  parentValidatorEtr: ValidatorEtr<ParentT, FailureT>
): ValidatorEtr<ValidatableT, FailureT> = ValidatorEtr { childValidatable ->
  parentValidatorEtr.apply(narrow(childValidatable))
}
...
fun <ParentT, ValidatableT : ParentT, FailureT> liftAllToChildValidatorType(
  ...
  parentValidatorEtrs.map { liftToChildValidatorType(it) }
