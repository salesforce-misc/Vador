...
internal fun <ValidatableT, FailureT> BaseValidationConfig<ValidatableT, FailureT>
  .getPredicateOfSpecForTestEx(nameForTest: String): Optional<Predicate<ValidatableT?>> {
...
internal fun <ValidatableT> BaseValidationConfig<ValidatableT, *>.getRequiredFieldNamesEx(
  ...
  (shouldHaveFieldsOrFailWith.keys +
      (shouldHaveFieldsOrFailWithFn?._1 ?: emptyList()) +
      shouldHaveFieldOrFailWithFn.keys)
    .map { PropertyUtils.getPropertyName(beanClass, it) }
    .toSet()

internal fun <ValidatableT> getValidatableType(
  config: BaseValidationConfig<ValidatableT, *>
): Type? {
  (config.withValidators?._1?.firstOrNull() ?: config.withValidator.keys.firstOrNull())?.let {
    return TypeResolver.resolveRawArguments(Validator::class.java, it.javaClass)[0]
  }
  ...
  config.getSpecsEx().firstOrNull()?.let {
    ...
    return when (it) {
      is Spec1<*, *, *> ->
        TypeResolver.resolveRawArguments(Function1::class.java, it.given.javaClass)[0]
      is Spec2<*, *, *, *> ->
        TypeResolver.resolveRawArguments(Function1::class.java, it.`when`.javaClass)[0]
      is Spec3<*, *, *, *, *> ->
        TypeResolver.resolveRawArguments(Function1::class.java, it.`when`.javaClass)[0]
      else -> null
    }
