...
Each `ContainerT` validity depends on all it's N nested `MemberT`\`s validity. But don't worry, the challenge is already solved by Vader. You just need to ask him to do it for you, by hooking `BatchValidationConfig<MemberT, FailureT>` into `BatchOfBatch1ValidationConfig<ContainerT, MemberT, FailureT>`.
...
[source,java,indent=0,options="nowrap"]
----
include::{testdir}/org/revcloud/vader/runner/config/nested/BatchOfBatch1ValidationConfigTest.java[tag=batch-of-batch-1]
----
...
[source,java,indent=0,options="nowrap"]
----
include::{testdir}/org/revcloud/vader/runner/config/nested/BatchOfBatch1ValidationConfigTest.java[tag=batch-of-batch-1-demo]
----
